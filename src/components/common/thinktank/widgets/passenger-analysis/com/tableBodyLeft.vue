...
<div id="tablebodyleft">
	...
	<div class="table-body">
		<div id="item-container" :style="{paddingTop:containerPaddingTop}">
			<div class="item" v-for="(item,index) in list" :key="index">
				...
				<div v-if="item.station.length > 7" class="cursor-pointer" @mouseenter="enter(item.station,$event)" @mouseleave='out'>{{item.station}}</div>
				<div v-else>{{item.station}}</div>
	...
	<div id="tip-msg-station-tablebodyleft">{{msg}}</div>
...
export default {
	...
	mounted(){
		this.tipDom = document.getElementById('tip-msg-station-tablebodyleft');
	},
	methods:{
		out(){
...
#item-container{
	width: 100%;
	height: 100%;
	transition: padding-top 2.5s ease-out;
	-webkit-transition: padding-top 2.5s ease-out;
	overflow:auto;
	box-sizing: border-box;
	pointer-events: all;
}
