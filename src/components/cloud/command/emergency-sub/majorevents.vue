...
export default {
	...
	watch:{
		showStatus(newVal, oldVal) {
			if(newVal == 1){
				let arr = JSON.parse(JSON.stringify(this.$store.getters["emergencyPlan/selectedMajorevents"]));
	...
	methods:{
		...
		sendSelectedIds(){ //发送并且存储已经启动项
			...
			//存储已选中项
			this.$store.dispatch('emergencyPlan/selectedMajorevents',JSON.parse(JSON.stringify(this.selectedIds)));
		}
	},
	computed:{
		...mapState('emergencyPlan',{
			supportList(state){
				if(state.majorevents.length){
					...
					this.$store.dispatch('emergencyPlan/majorevents_buses',qiDong);
		...
		...mapGetters("emergencyPlan", ["majorevents_buses"]),
		involving_companies(){
			let majorevents_buses = this.majorevents_buses;
			if(!majorevents_buses.length){
				return false;
			}

			//统计已启动线路应急支援车辆所属公司有哪些，在界面上高亮有车辆投入的公司
			console.log('majorevents_buses--',majorevents_buses)
			let buses_involving_companies = {}; //存放已启动线路应急支援车辆所属公司
			let len = majorevents_buses.length;

			for(let i=0;i<len;i++){

				let line_itemId = majorevents_buses[i].itemId;
				buses_involving_companies[line_itemId] = [];

				let list_item = majorevents_buses[i].list;
