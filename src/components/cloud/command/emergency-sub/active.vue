...
<div id='active'> <!-- 大型活动 -->
	...
	<div @click="navTo()" class="style-com style-1 color1">
		...
		<div class="yingJi-item-com"
			...
			<div class="d1-com d1-color1">进博会</div>
			<div class="d-com">
				<div class="d-com-title">配套线路</div>&nbsp;&nbsp;
				<div class="val-1">65/26条</div>
			</div>
			<div class="d-com">
				<div class="d-com-title">配套车辆</div>&nbsp;&nbsp;
				<div class="val-1">0/0/0辆</div>
			</div>
			<div class="d4">
				<div class="join-company-com join-company-title">参与公司</div>
				<div class="join-company-com d4-val-2">一公司</div>
				<div class="join-company-com d4-val-2">二公司</div>
				<div class="join-company-com d4-val-2">三公司</div>
				<div class="join-company-com d4-val-2">四公司</div>
				<div class="join-company-com d4-val-2">五公司</div>
			</div>
		</div>
	</div>
<!-- 其它活动 -->
...
export default {
	...
	watch:{
		showStatus(newVal, oldVal) {
			if(newVal == 2){
				let arr = JSON.parse(JSON.stringify(this.$store.getters["emergencyPlan/selectedActive"]));
	...
	methods:{
		...
		sendSelectedIds(){ //发送并且存储已经启动项
			...
			//存储已选中项
			this.$store.dispatch('emergencyPlan/selectedActive',JSON.parse(JSON.stringify(this.selectedIds)));
		}
	},
	computed:{
		...mapState('emergencyPlan',{
			supportList(state){
				if(state.active.length){
					let data = JSON.parse(JSON.stringify(state.active[0].supportList));
					...
					this.$store.dispatch('emergencyPlan/get_lines',{'event_name':'active_buses','params':qiDong});
