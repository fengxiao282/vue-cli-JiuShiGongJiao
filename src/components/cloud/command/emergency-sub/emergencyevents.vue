...
export default {
	...
	watch:{
		showStatus(newVal, oldVal) {
			if(newVal == 0){
				let arr = JSON.parse(JSON.stringify(this.$store.getters["emergencyPlan/selectedEmergencyevents"]));
	...
	methods:{
		...
		sendSelectedIds(){ //发送并且存储已经启动项
			...
			//存储已选中项
			this.$store.dispatch('emergencyPlan/selectedEmergencyevents',JSON.parse(JSON.stringify(this.selectedIds)));
		}
	},
	computed:{
		...mapState('emergencyPlan',{
			supportList(state){
				if(state.emergencyevents.length){
					let data = JSON.parse(JSON.stringify(state.emergencyevents[0].supportList));
					...
					this.$store.dispatch('emergencyPlan/emergencyevents_buses',qiDong);
		...
		...mapGetters("emergencyPlan", ["emergencyevents_buses"]),
		involving_companies(){
			let emergencyevents_buses = this.emergencyevents_buses;
			if(!emergencyevents_buses.length){
				return false;
			}

			//统计已启动线路应急支援车辆所属公司有哪些，在界面上高亮有车辆投入的公司
			console.log('emergencyevents_buses--',emergencyevents_buses)
			let buses_involving_companies = {}; //存放已启动线路应急支援车辆所属公司
			let len = emergencyevents_buses.length;

			for(let i=0;i<len;i++){

				let line_itemId = emergencyevents_buses[i].itemId;
				buses_involving_companies[line_itemId] = [];

				let list_item = emergencyevents_buses[i].list;
...
#emergencyevents::-webkit-scrollbar{
	display: none;
}
#emergencyevents > div{
	cursor: pointer;
