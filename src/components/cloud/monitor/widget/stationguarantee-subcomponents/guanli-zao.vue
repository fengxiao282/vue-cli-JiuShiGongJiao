<template>
	<div id='baoanBaoJie-zao'>
		<investitemtitle titleName="管理人员早上岗">
			<div slot="content" class="screen-count"></div>
		</investitemtitle>

        <div class="bottom-container">
			<div class="left" id='baoanBaoJie_zao_pie' :style="{transform: 'scale('+scale+')'}"></div>
			<div class="right">
  ...
  updated(){
	...
	methods:{
		...
		initPie(newVal){ //初始化饼图
			...
			let myChart = echarts.init(document.getElementById("baoanBaoJie_zao_pie"));
		...
		...mapState('stationguarantee',{
			zaoShangGang(state){
				if(state.zaowan_shanggang){
					let data = state.zaowan_shanggang.am.manager;
