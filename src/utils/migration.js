...
var Arc = (function () {
    var A = function (options) {
        ...
        if (options.direction == 'clockwise') {
            direction = false
            centerX = (startY - endY) * factor + m
            centerY = (endX - startX) * factor + n
        } else {
            direction = true
            centerX = (-startY + endY) * factor + m
            centerY = (-endX + startX) * factor + n
...
var Spark = (function () {
    ...
    S.prototype.drawArc = function (context, strokeColor, lineWidth, startAngle, endAngle) {
        ...
        context.beginPath()
        context.arc(this.centerX, this.centerY, this.radius, startAngle, endAngle, this.direction)
        context.stroke()
        context.restore()
    }
    ...
    S.prototype.draw = function (context) {
        ...
        if (this.direction) {
            if ((this.trailAngle - endAngle) * 180 / Math.PI < 0.5) {
                this.trailAngle = this.startAngle
    ...
    if (this.data && this.data.length > 0) {
        arrayUtils.forEach(this.data, function (element) {
            var arc = new Arc({
                ...
                direction: element.direction || 'clockwise',
            })
